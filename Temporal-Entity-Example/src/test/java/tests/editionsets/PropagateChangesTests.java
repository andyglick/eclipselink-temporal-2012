...
public class PropagateChangesTests extends BaseTestCase
{
  ...
  public void makeBasicChangesAtT2Changes()
  {
    ...
    String nativeName = (String) em.createNativeQuery("SELECT P_NAMES FROM TPERSON WHERE CID = " + getSample().getId() + " AND START_TS = 400")
      .getSingleResult();
    Assert.assertEquals("Jimbo", nativeName);
    Assert.assertEquals("Jimbo", personT4.getName());

    String nativeState = (String) em.createNativeQuery("SELECT STATE FROM TADDRESS WHERE CID = " + getSample().getAddress().getId() + " AND START_TS = 400")
      .getSingleResult();
